...
namespace KeraLuaLite
{
    ...
    static partial class Lua
    {
        ...
        {
            return NativeMethods.LuaPCall(luaState, nArgs, nResults, errfunc);
        }
        ...
        public static double LuaToNumber(IntPtr luaState, int index)
        {
            return NativeMethods.LuaToNumber(luaState, index);
        }
        ...
        public static void LuaNetPushStdCallCFunction(IntPtr luaState, LuaNativeFunction fn)
        {
            IntPtr pfunc = Marshal.GetFunctionPointerForDelegate(fn);
            NativeMethods.LuaNetPushStdCallCFunction(luaState, pfunc);
        }
        ...
        [CLSCompliantAttribute(false)]
        public static int LuaLLoadBuffer(IntPtr luaState, byte[] buff, uint size, string name)
        {
            return NativeMethods.LuaLLoadBuffer(luaState, buff, size, name);
        }

        public static int LuaLLoadFile(IntPtr luaState, string filename)
        {
            return NativeMethods.LuaLLoadFile(luaState, filename);
        }
        ...
        public static bool LuaNetCheckMetatable(IntPtr luaState, int obj)
        {
            return NativeMethods.LuaNetCheckMetatable(luaState, obj) != 0;
        }
        ...
        // - mouguangyi
        public static int LuaObjLen(IntPtr luaState, int index)
        {
            return NativeMethods.LuaObjLen(luaState, index);
        }

        public static void LuaPushLString(IntPtr luaState, byte[] str, int size)
        {
            NativeMethods.LuaPushLString(luaState, str, size);
        }

        public static void LuaPushCClosure(IntPtr luaState, LuaNativeFunction function, int n)
        {
            IntPtr pfunc = Marshal.GetFunctionPointerForDelegate(function);
            NativeMethods.LuaPushCClosure(luaState, pfunc, n);
        }

        public static int LuaNetUpValueIndex(int index)
        {
            return NativeMethods.LuaNetUpValueIndex(index);
        }
    }
}